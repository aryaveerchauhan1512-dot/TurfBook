...
function readDb(): any {
  ...
  if (!Array.isArray(memoryDb.conversations)) memoryDb.conversations = [];
  if (!Array.isArray(memoryDb.chatMessages)) memoryDb.chatMessages = [];
  return memoryDb;
}
...
app.post('/api/turfs/:id/slots/block', (req, res) => {
  const { slotId, status, bookedByUserName, bookedByUserPhone } = req.body || {};
  const db = readDb();
  db.slots = db.slots || [];
